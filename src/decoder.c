...
void decode_settings(char *args)
{
    ...
    else if (strcmp(group, "firmware") == 0) 
    {
        if (strcmp(tag, "version") == 0) 
        {
            settings_set(KEY_SENSOR_FIRMWARE_VERSION, value, strlen(value), true);
            ESP_LOGI(TAG,"Firmware version set to: %s\n", s_settings.firmware_version);
        } 
        else 
        {
            ESP_LOGE(TAG,"Unknown sensor tag: %s\n", tag);
...
void query_settings(char *args)
{
    ...
    else if (strcmp(group, "firmware") == 0) 
    {
        if (strcmp(tag, "version") == 0) 
        {
            sprintf(query_mqtt_msg, "Firmware version: %s\n", s_settings.firmware_version);
            ESP_LOGI(TAG,"%s", query_mqtt_msg);
        }
        else 
        {
